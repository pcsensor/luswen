...
- 发布态过滤、排序、标签聚合和相邻文章计算统一由 `src/lib/posts.ts` 提供。
- 页面不得直接复制排序、过滤或 URL 拼装规则。
- RSS、Sitemap 与 HTML 页面共享同一内容查询层，避免结果不一致。
...
例如分类统计、系列文章或相关推荐，应先在 `src/lib/posts.ts` 添加纯函数，再由页面调用。
...
1. `npm run check:architecture` 通过。
2. `astro check` 无错误。
3. `npm run build` 成功生成所有 HTML 与 XML 路由。
4. 桌面端和移动端没有横向溢出、遮挡或不可达导航。
5. 首页、至少一篇文章、404、RSS、Sitemap 和 robots.txt 可访问。
6. Git 工作区只包含本次任务相关变更。

## 11. 决策记录

- 采用静态输出：博客内容更新频率低，静态文件更快、更易部署，也减少运行时故障面。
- 不引入 CMS：内容保留在仓库内，版本历史清晰；以后需要多人协作时可在内容边界替换数据源。
- 不引入搜索依赖：当前内容规模较小，标签和归档即可完成发现；文章数量显著增长后再评估 Pagefind 等构建期索引。
- 不绑定评论与分析供应商：在没有明确隐私和服务选择前，不植入第三方追踪或外部状态。
